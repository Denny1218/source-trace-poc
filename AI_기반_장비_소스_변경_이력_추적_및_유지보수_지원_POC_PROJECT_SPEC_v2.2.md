...
## PROJECT_SPEC v2.2 (현재 기준 명세)

- 문서 성격: 현재 구현·운영 기준의 통합 명세
- 작성 기준일: 2026-08-05
- 기반 문서: `AI_기반_장비_소스_변경_이력_추적_및_유지보수_지원_POC_PROJECT_SPEC_v2.1.md`
- 이전 기준: PROJECT_SPEC v2.1
- 우선순위: 본 문서(v2.2)가 현재 작업·검증·산출물의 최우선 기준이다. 이전 v2.1 및 기존 보완명세와 충돌하면 본 문서를 따른다.
- STEP 10: 별도 승인 없이 진행하지 않는다.
- v2.2 핵심 정책: Git lifecycle과 관련 공식 문서는 별도 축으로 관리한다. 문서를 `최초 추가/공식 적용/후속 기능 변경/후속 유지보수` 역할로 추정 분류하지 않는다. 문서와 Git의 관계는 `Commit 직접 근거/단계 연결 근거/관련 참고`의 연결 강도로만 표시하며, 날짜 임계값(예: 270일)은 문서 역할 또는 연결 유형 판정에 사용하지 않는다.
...
PROJECT_SPEC v2.2 (본 문서)
  → v2.1을 기반으로 Git lifecycle과 관련 공식 문서의 독립 관리 정책을 반영한 현재 기준 명세
```
...
- VS Code Extension에서 선택 코드/함수의 변경 lifecycle 및 변경 사유 조회
- (선택) VSCode Continue에서 동일 Backend API로 자연어 질의
...
선택 호환
VSCode Continue (config 수동 반영, Extension 비필수)
```
...
Backend는 Extension·Continue와 분리된 HTTP Service API로 구현한다.

## Continue 연계 (선택 호환)

Continue는 필수 런타임이 아니다. 사용 시 Local HTTP API 호출 방식으로 Backend에 연결한다.

MCP / Custom Tool / Context Provider는 내부 Continue 버전에서 지원될 때만 검토한다. 특정 연계 방식을 사전에 확정하여 코드 전체를 종속시키지 않는다.

Extension은 Continue `config.yaml`을 자동 수정하지 않고, 삽입용 문구 생성만 지원할 수 있다.
...
├── AI_기반_...PROJECT_SPEC_v2.1.md   # 이전 기준
├── AI_기반_...PROJECT_SPEC_v2.2.md   # 현재 기준
├── TEST_PLAN.md
├── DEPLOYMENT.md
└── README.md
```
...
true일 때만 민감하지 않은 내부 진단 정보를 출력하며, 선택 코드 원문·전체 Diff·토큰·비밀번호는 출력하지 않는다.

Continue 요청의 Source Trace 분석 진행 상태를 Output에 표시할 수 있으나, Backend Context 생성 완료와 Continue AI 최종 답변 완료를 혼동하지 않는다.

### Continue Output 정책 (요약 모드 기본)

Workspace당 polling loop는 하나만 유지한다. Extension activation 시 기존 timer를 정리한 뒤 새 timer를 등록한다.

요청마다 Backend가 `request_id`(형식: `CT-YYYYMMDD-HHMMSS-XXXX`)와 단조 증가하는 `request_sequence`를 부여한다.

기본 Output(`sourceTrace.continueProgressDetail = summary`)은 요청당 다음만 1회씩 출력한다.

```text
[HH:MM:SS] ── Continue 요청 시작 [CT-XXXX] ──
[HH:MM:SS] Symbol: <symbol>

[HH:MM:SS] ── Source Trace 처리 완료 [CT-XXXX] ──
[HH:MM:SS] 소요 시간: N초 · Context M자 · Continue 전달 완료
[HH:MM:SS] Continue 최종 답변 생성은 Continue AI 모델에서 별도로 수행됩니다.
```

실패 시:

```text
[HH:MM:SS] ── Source Trace 처리 실패 [CT-XXXX] ──
[HH:MM:SS] 오류: ...
```

summary 모드에서 출력하지 않는 항목:
...
문서 연결 분석 중
Context 생성 중
동일 request_id + 동일 status 반복
과거 completed 요청 재출력
```

첫 polling은 현재 `request_sequence`를 baseline으로 저장하고 이전 완료 이벤트는 출력하지 않는다. 이후 `GET /api/continue/status?client_id=...&after_sequence=N` 증분 조회를 사용한다.

`sourceTrace.continueProgressDetail = debug`일 때만 polling 단계·내부 상태를 추가 출력한다.
...
Source Trace 처리 완료
Context 전달 완료
(금지) Continue 요청 완료 / AI 답변 완료
```

## 12.6 Continue 결과 정책

Continue는 선택적 호환 기능이며 **정확한 전체 변경 이력의 공식 기준은 Extension 직접 조회 결과**다.

```text
단순 변경 이력 조회 / 명시적 요약·설명 요청
→ Backend가 결정론적으로 생성한 짧은 근거 Context ([SOURCE TRACE CONFIRMED EVIDENCE])
→ 전체 Extension Markdown(표·details·중첩 구조)은 기본 전달하지 않음
→ Backend Ollama는 단순 조회에서 기본 우회 (use_ollama=false)
→ Continue HTTP Context Provider는 모델 없는 직접 출력이 불가능함을 전제로 함

일반 질의(비 이력 조회)
→ 기존 보호 지시 + 전체 Markdown Context 유지 가능
```

간략 Context에 포함:

```text
대상 Symbol, Extension 직접 조회가 공식 기준이라는 안내
최초 확인 Commit, 주요 Commit 목록(날짜·연결 유형·한 줄 요약)
관련 공식 문서(문서명·Slide·CSR·버전·대표/Commit별 연결 유형)
분석상 주의사항
```
...
```text
중첩 Markdown 표, <details>, 관련 소스/함수 전체 목록
전체 Extension 보고서 원문, 역할 추론 가능한 장문 서술
```

관련 공식 문서 섹션에서 Commit별 연결 유형을 함께 표시한다(대표 연결 유형만으로 모든 Commit이 직접 근거인 것처럼 보이지 않게).

연결 근거 문구:

```text
단계 연결(Diff 미확보): Commit 메시지·문서 주제 일치 + 관련 함수 확인, 세부 Diff 미확보
관련 참고: 직접 Diff 연결 없음을 객관적으로 기술 (후속/유지보수 역할 추정 표현 금지)
```

Continue 보호 지시에는 다음을 포함한다.

```text
문서를 최초 적용·후속 적용으로 재분류하지 않음
연결 유형을 그대로 유지
관련 참고를 Commit 직접 근거로 변경하지 않음
문서 순서나 날짜로 역할을 추론하지 않음
원문에 없는 함수 역할·코드값·신뢰도·영향도를 생성하지 않음
날짜·Commit·Slide·CSR·버전 변경 금지
Markdown 표·details 재구성 금지
```

## 12.7 README 및 사용자 문서
...
2. 한눈에 보기에서 `공식 적용/후속 공식 기능 변경/후속 유지보수 문서` 단계 표현을 사용하지 않는다.
...
9. 본문에 사용된 모든 문서가 참조 근거와 집계에 포함된다.
10. 동일 문서·Slide의 중복 집계가 없다.
11. 관련 소스와 관련 함수가 분리된다.
12. 경로 토큰이 함수로 출력되지 않는다.
13. Output 기본 모드에 내부 디버그 페이로드가 없다.
14. Output 문서 통계가 관련 공식 문서 컬렉션과 일치한다.
15. Continue Context가 문서 역할을 최초/후속으로 재분류하지 않는다.
16. Extension 서버·장비 설정 후 Continue config 문구를 생성할 수 있다.
17. Continue config는 자동 수정하지 않는다.
18. 기존 Git lifecycle 및 일반 검색 회귀가 없다.
...
Extension/Continue 연계와 별도로 Backend 단독으로 "변경 이유 추적 요청"을 받을 수 있는 핵심 Trace API를 구현한다.
...
Extension 또는 Continue에서 현재 파일 또는 선택 코드 정보를 전달할 수 있을 때 사용한다.
...
# STEP 9. VS Code Extension 연계 (주 인터페이스) 및 Continue 호환
...
개발자가 VS Code에서 선택한 함수/코드의 Git 변경 lifecycle과 관련 공식 문서를 서로 구분하여 조회할 수 있도록 한다.

주 경로:

```text
Source Trace VS Code Extension
→ POST /api/continue/trace (또는 동일 계약의 분석 API)
→ Git lifecycle + 관련 공식 문서 Markdown
```

Continue 연동은 선택적 호환이다. Extension 사용에 Continue 설치를 필수로 요구하지 않는다.

## Extension 핵심 기능

```text
함수 또는 코드 선택 후 장비 변경 이력 조회
서버 및 장비 설정 (장비명 Quick Pick)
서버 연결 확인 / 장비 변경 / 현재 설정 보기
분석 진행 Output Channel
결과 Markdown 문서 표시
(선택) Continue 설정 문구 생성·복사 — config.yaml 자동 수정 금지
```
...
Extension / Continue Integration Layer는 다음 API를 호출한다.

```http
POST /api/continue/trace
```

호환·검증용으로 Trace 계열 API(`POST /api/trace/analyze` 등)도 유지할 수 있다.
...
- Extension이 생성한 설정 문구를 사용자가 `config.yaml`에 수동 반영할 수 있다.
- Extension은 Continue 설정 파일을 자동 수정하지 않는다.
- 내부 Continue 버전·MCP/Tool 지원 여부는 운영 환경에서 별도 확인한다. Backend는 HTTP Service로 분리 유지한다.
...
Extension 실제 사용 시나리오 테스트 (Continue는 선택):
...
Extension README에 STEP/MVP/POC 데모·과거 Continue 비교 이력 유지
```
...
Continue를 Extension 대체로 되돌리는 작업
```

신규 작업 시 본 문서(PROJECT_SPEC v2.2)를 최우선으로 참고하고, 완료 보고 형식(§19)과 산출물 원칙(§12.7)을 따른다.
