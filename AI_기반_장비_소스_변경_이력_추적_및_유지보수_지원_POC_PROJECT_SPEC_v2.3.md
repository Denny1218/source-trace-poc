...
## PROJECT_SPEC v2.3 (현재 기준 명세)

- 문서 성격: 현재 구현·운영 기준의 통합 명세
- 작성 기준일: 2026-08-05
- 기반 문서: `AI_기반_장비_소스_변경_이력_추적_및_유지보수_지원_POC_PROJECT_SPEC_v2.2.md`
- 이전 기준: PROJECT_SPEC v2.2
- 우선순위: 본 문서(v2.3)가 현재 작업·검증·산출물의 최우선 기준이다. 이전 v2.2 및 기존 보완명세와 충돌하면 본 문서를 따른다.
...
- v2.3 핵심 정책: Continue 연동을 프로젝트 공식 범위에서 제거한다. 공식 사용자 경로는 Source Trace VS Code Extension 직접 조회이며, Continue Context Provider·설정 스니펫·상태 API·polling·진행 Output·관련 문서와 테스트를 제거한다. 내부 오프라인 소형 모델의 출력은 공식 근거로 사용하지 않는다.
...
PROJECT_SPEC v2.3 (본 문서)
  → v2.2를 기반으로 Continue 연동을 공식 범위에서 제거하고 Source Trace Extension 단독 운영 정책을 반영한 현재 기준 명세
```
...
Continue는 PROJECT_SPEC v2.3의 공식 기능과 운영 범위에서 제외한다.
...
├── AI_기반_...PROJECT_SPEC_v2.3.md   # 현재 기준
├── TEST_PLAN.md
├── DEPLOYMENT.md
└── README.md
```
...
## 12.6 Continue 연동 제외 정책

Continue 결과는 본 프로젝트의 공식 결과가 아니다. PROJECT_SPEC v2.3에서는 Continue Context 생성·전달·상태 추적·Output 표시를 제공하지 않는다.
...
15. 16. 17. 18. 기존 Git lifecycle 및 일반 검색 회귀가 없다.
...
```text
Source Trace VS Code Extension
→ POST /api/trace/analyze (중립 Trace API)
...
Source Trace Extension은 다음 중립 Trace API를 호출한다.

```http
POST /api/trace/report
```

> 구현 시점에 `POST /api/trace/analyze`는 STEP 8 Ollama Evidence Grounded Answer용
> `backend/app/api/analysis.py` 엔드포인트가 이미 사용 중이어서(응답 스키마도 다름) 충돌을
> 피하기 위해 `POST /api/trace/report`(`backend/app/api/trace_extension.py`)로 확정했다.
> 기존 `/api/continue/trace`는 Continue 전용 상태 추적·Context wrapper와 함께 완전히
> 제거했으며, 호환 alias는 유지하지 않는다(신규 Extension 빌드만 배포 대상이므로).
...
신규 작업 시 본 문서(PROJECT_SPEC v2.3)를 최우선으로 참고하고, 완료 보고 형식(§19)과 산출물 원칙(§12.7)을 따른다.
