...
- v2.4 핵심 정책: 함수/Symbol 전체 이력 조회와 선택 코드(라인·블록) 변경 근거 조회를 분리한다. 선택 코드 조회는 키워드 후보 검색이 아니라 Git blame과 line history를 우선 사용하며, 직접 근거가 없는 Commit·PPT는 결과에 승격하지 않는다. Continue 제거 과정에서 사라진 일반 Source Trace Output을 복구하고 조회 모드별 시작·완료 로그를 유지한다.
...
PROJECT_SPEC v2.4 (본 문서)
  → v2.3을 기반으로 함수 전체 이력 조회와 선택 코드 변경 근거 조회를 분리하고, line-level Git 근거·분류 정확도·Output 복구 정책을 반영한 현재 기준 명세
```
...
├── AI_기반_...PROJECT_SPEC_v2.4.md   # 현재 기준
├── TEST_PLAN.md
├── DEPLOYMENT.md
└── README.md
```
...
변경 성격(카드 유형, 재승차, 기후동행, 날짜 비교 등)은 다음 순서로 판정한다.

```text
1. 대상 함수 범위 내 실제 Diff(변경된 줄)에서 확인된 내용
2. Commit 메시지 (Diff가 없거나 Diff만으로 성격을 확정할 수 없을 때)
3. 보조 휴리스틱(정렬·주의 표시용)
```
...
```text
Diff와 Commit 메시지가 서로 다른 변경 성격을 가리키면 Diff를 우선한다.
Diff를 확보하지 못한 경우에만 Commit 메시지로 보수적으로 분류한다.
```

금지:

```text
Commit 메시지만으로 Diff와 반대되는 변경 성격을 확정
Diff에 없는 카드/생년월일 토큰(맥락 줄)만으로 재승차·기후동행 등을 덮어쓰기
```
...
`time_band`(near / mid / far)는 후보 **탈락** 조건으로 사용하지 않는다. 날짜·시점 정보는 다음 용도로만 쓴다.

```text
후보 정렬 및 우선순위 보조
연결 신뢰도(높음/중간/낮음) 조정
사용자 주의 문구(시점 차이가 큼 등)
```

단계 연결·Commit 직접 근거·관련 참고의 **필수** 판정은 다음 명시적 근거로 한다.

```text
CSR / 적용 버전 / 릴리스 범위
Commit 메시지와 문서 As-Is/To-Be·주요 변경 내용의 기능 연속성
대상 함수·파일 Diff와 문서 변경 행위 일치
문서 관련 함수·소스 목록과 대상 Symbol 일치
```

금지:

```text
270일·365일·540일 등 고정 임계값으로 문서 역할 결정
time_band가 far이면 단계 연결 후보를 무조건 제외
가장 오래된 문서를 최초 적용 문서로 자동 승격
최초 Commit과 가까운 문서를 최초 추가 문서로 자동 승격
문서 작성일이 늦다는 이유만으로 후속 유지보수 문서로 단정
날짜·제목만으로 Commit 직접 근거 또는 단계 연결 근거 승격
```
...
11. Diff와 Commit 메시지가 충돌하면 Diff 기준 변경 성격이 우선한다.
12. Commit 메시지가 재승차·기관·시간 조건인데 Diff 맥락의 무관 토큰만으로 `카드 사용자 유형`, `생년월일` 등으로 오분류되지 않는다.
13. Diff로 세부 성격을 확정할 수 없으면 `조건 분기 변경` 또는 `세부 Diff 확인 필요`처럼 보수적으로 표시한다.
14. 대상 함수 직접 Diff가 없는 Commit은 핵심 변경이 아니라 연관 후보로 분리한다.
15. 문서 작성일 이후의 Commit을 동일 단계로 연결할 때는 기능·버전·CSR·Diff의 명시적 근거가 필요하다.
16. `time_band`가 far이어도 명시적 근거가 있으면 단계 연결을 막지 않는다(날짜만으로 탈락 금지).
17. 문서 관련 함수 목록에 대상 함수가 없으면 `대상 함수가 관련 함수로 확인됨` 문구를 생성하지 않는다.

### 선택 코드 조회

18. 선택 행 범위가 Backend로 정확히 전달된다.
19. `git blame -L` 결과의 Commit이 현재 라인 근거로 표시된다.
20. blame Commit의 실제 Diff가 확인된다.
21. line history가 가능한 경우 과거 변경 흐름이 표시된다.
22. line history 추적 실패 시 제한 사유가 표시된다.
23. 키워드 score 1위 Commit을 line 근거로 대체하지 않는다.
24. 선택 코드와 직접 연결되지 않은 PPT를 대표 문서로 표시하지 않는다.
25. 직접 연결 문서가 없으면 없음으로 표시한다.
26. 한 줄 선택과 여러 줄 블록 선택을 모두 지원한다.
27. 선택이 없거나 공백만 있으면 사용자에게 올바른 선택을 안내한다.
28. 선택 범위가 함수 밖이거나 여러 함수를 걸치면 포함 범위를 명시한다.
29. 함수 전체 이력과 선택 코드 직접 근거를 같은 결과로 혼합하지 않는다.

### Output·회귀

30. `Source Trace` Output Channel이 Extension 활성화 후 존재한다.
31. 함수 조회 시작·완료 로그가 출력된다.
32. 선택 코드 조회 시작·blame·완료 로그가 출력된다.
33. Continue 제거 후에도 일반 Output이 유지된다.
34. 기본 Output에 선택 코드 원문·전체 Diff·민감 정보가 없다.
35. Output 문서 통계가 관련 공식 문서 컬렉션과 일치한다.
36. 기존 Git lifecycle·장비 설정·서버 설정·Markdown 저장 기능 회귀가 없다.
...
`/api/trace/report`는 기존 STEP 4 Web UI `/api/trace/search`와 별도 Endpoint이며 Extension 함수/Symbol 조회 전용이다. 신규 `/api/trace/selection`은 선택 코드 조회 전용으로 완전히 분리된 Schema·Service를 사용한다. Continue 전용 `/api/continue/trace`는 v2.3에서 이미 제거되었으며 재도입하지 않는다.
...
신규 작업 시 본 문서(PROJECT_SPEC v2.4)를 최우선으로 참고하고, 완료 보고 형식(§19)과 산출물 원칙(§12.10)을 따른다.
