...
## PROJECT_SPEC v2.5.1 (현재 기준 명세)

- 문서 성격: 현재 구현·운영 기준의 통합 명세
- 작성 기준일: 2026-08-07
- 기반 문서: `AI_기반_장비_소스_변경_이력_추적_및_유지보수_지원_POC_PROJECT_SPEC_v2.5.md`
- 이전 기준: PROJECT_SPEC v2.5
- 우선순위: 본 문서(v2.5.1)가 현재 작업·검증·산출물의 최우선 기준이다. 이전 버전과 충돌하면 본 문서를 따른다.
...
- v2.5.1 핵심 정책: 함수 Diff 근거 4단계(`DIRECT_BODY_CHANGE` / `FUNCTION_CONTEXT_CHANGE` / `SYMBOL_ONLY` / `MESSAGE_ONLY`), Repo remote URL canonicalization·재사용, 관련 목록 표시 우선순위.
...
PROJECT_SPEC v2.5.1 (본 문서)
  → v2.5를 기반으로 함수 Diff 판정 세분화, 선택 코드 Repo 매칭 보완, 관련 목록 표시 우선순위를 추가한 현재 기준 명세
```
...
├── AI_기반_...PROJECT_SPEC_v2.5.1.md   # 현재 기준
├── TEST_PLAN.md
├── DEPLOYMENT.md
└── README.md
```
...
49. 실제 대상 함수 body hunk가 존재하면 `DIRECT_BODY_CHANGE` 또는 `FUNCTION_CONTEXT_CHANGE`로 판정된다.
50. Allman 스타일 함수(`func()\n{`)의 변경 hunk를 함수 본문으로 인식한다.
51. 다중 라인 함수 시그니처의 body 변경을 Symbol-only로 강등하지 않는다.
52. `SYMBOL_ONLY`인데 `Diff상 대상 함수 기능 변경`이라고 단정하지 않는다.
53. Commit 메시지만 관련인 경우 `MESSAGE_ONLY`로 남고 주요 변경으로 승격되지 않는다.
54. 함수 변경 hunk는 확인되나 parser가 완전하지 않은 경우 `FUNCTION_CONTEXT_CHANGE`로 보수적으로 표시한다.
55. 선택 코드 조회는 기존 장비/함수 조회의 Repo 식별 정보를 재사용할 수 있다.
56. SCP/SSH/HTTPS 형식 차이만 있는 동일 remote URL을 canonical Repository로 매칭한다.
57. Repo 매칭 실패 시 `서버 실행 상태와 API 주소를 확인하세요` 같은 고정 오안내를 출력하지 않는다.
58. 서버/API 연결 실패와 Repo 매칭 실패의 Output 안내가 구분된다.
59. 관련 함수 목록이 표시 제한을 초과해도 현재 조회 대상 함수가 원본 목록에 있으면 화면에 반드시 표시된다.
60. 관련 소스 목록이 표시 제한을 초과해도 현재 조회 대상 파일이 원본 목록에 있으면 화면에 반드시 표시된다.
61. 표시 목록 제한이 Evidence Link 판정용 전체 related_functions/related_sources 데이터에 영향을 주지 않는다.
62. 표시 생략 건수는 `외 N개`로 사용자에게 알린다.
...
신규 작업 시 본 문서(PROJECT_SPEC v2.5.1)를 최우선으로 참고하고, 완료 보고 형식(§19)과 산출물 원칙(§12.10)을 따른다.
