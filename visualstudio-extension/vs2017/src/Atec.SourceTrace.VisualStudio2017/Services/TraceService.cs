...
namespace Atec.SourceTrace.VisualStudio2017.Services
{
    public sealed class TraceService
    {
        ...
        public async ThreadingTasks.Task RunFunctionHistoryAsync()
        {
            string cfgMsg;
        ...
        public async ThreadingTasks.Task RunSelectionTraceAsync()
        {
            string cfgMsg;
            if (!_settings.EnsureConfigured(out cfgMsg))
            {
        ...
        private async ThreadingTasks.Task SubmitSelectionAsync(
        ...
        public async ThreadingTasks.Task CheckServerAsync()
        {
            ...
            var result = await ThreadingTasks.Task.Run(() => _http.Health(norm.Url));
            if (result.Ok)
            {
        ...
        public async ThreadingTasks.Task ConfigureAsync()
        {
            var url = _settings.GetServerUrl();
            if (string.IsNullOrWhiteSpace(url))
            {
            ...
            try
            {
                var items = await ThreadingTasks.Task.Run(() => _http.ListEquipment(norm.Url));
                if (items.Count == 0)
                {
        ...
        private async ThreadingTasks.Task PostReportAsync(string jsonBody)
        {
            var serverUrl = ServerUrlUtil.NormalizeServerUrl(_settings.GetServerUrl()).Url;
            var result = await ThreadingTasks.Task.Run(() => _http.PostReport(serverUrl, jsonBody));
            await HandleResultAsync(result, "함수 변경 이력", false, null);
        }

        private async ThreadingTasks.Task PostSelectionAsync(string jsonBody, Func<ThreadingTasks.Task> onAmbiguityRetry)
        {
            var serverUrl = ServerUrlUtil.NormalizeServerUrl(_settings.GetServerUrl()).Url;
            var result = await ThreadingTasks.Task.Run(() => _http.PostSelection(serverUrl, jsonBody));
            await HandleResultAsync(result, "선택 코드 변경 근거", true, onAmbiguityRetry);
        }

        private async ThreadingTasks.Task HandleResultAsync(
            TraceHttpClient.HttpResult r,
            string titleHint,
            bool allowAmbiguityRetry,
            Func<ThreadingTasks.Task> onAmbiguityRetry)
        {
            if (r.Error != null)
            {
        ...
        private async ThreadingTasks.Task ShowMarkdownAsync(string markdown, string titleHint)
        {
            await _package.JoinableTaskFactory.SwitchToMainThreadAsync();
            var window = _package.FindToolWindow(typeof(SourceTraceToolWindow), 0, true) as SourceTraceToolWindow;
            if (window == null)
            {
                await ShowErrorAsync("ATEC Source Trace Tool Window를 열 수 없습니다.");
                return;
            }
        ...
        private async ThreadingTasks.Task<int?> PickRepoHintAsync()
        {
            var serverUrl = ServerUrlUtil.NormalizeServerUrl(_settings.GetServerUrl()).Url;
            var equipmentId = _settings.GetEquipmentId();
            var repos = await ThreadingTasks.Task.Run(() => _http.ListRepositories(serverUrl, equipmentId));
            if (repos.Count == 0)
            {
        ...
        private async ThreadingTasks.Task<TraceHttpClient.EquipmentItem> PickEquipmentAsync(
