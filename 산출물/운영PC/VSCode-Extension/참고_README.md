...
조회 결과는 **Untitled Markdown 문서**(언어 모드 `markdown`)로 열리고, 자동 Markdown Preview는 열리지 않습니다. 필요하면 VS Code 기본 Preview를 직접 엽니다. 디스크에
`.md`로 자동 저장하지 않으며, 저장 여부는 사용자가 결정합니다. 연속 조회마다 새 Untitled를
추가하고 이전 결과는 보존하며, 이전 Untitled 결과는 그대로 둡니다.
